        ...
        try {
            ...
            while (entryEnum.hasMoreElements()) {
                ...
                if (name.startsWith(rootEntryName) && name.indexOf('/', rootEnd) == -1 && name.endsWith(CLASS_FILE_EXTENSION)) {
                    ...
                    result.add(new CustomJavaFileObject(binaryName, uri, new FileOutputStream(uri.toURL().getFile())));
                }
            }
        } catch (Exception e) {
        ...
        for (File childFile : childFiles) {
            if (childFile.isFile()) {
                ...
                if (childFile.getName().endsWith(CLASS_FILE_EXTENSION)) {
                    ...
                    try {
                        result.add(new CustomJavaFileObject(binaryName, childFile.toURI(), new FileOutputStream(directory)));
